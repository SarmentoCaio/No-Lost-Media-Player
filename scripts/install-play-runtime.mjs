...
const enabled = process.env.VITE_PS2_ENABLED === "true" || process.argv.includes("--force");
...
async function installRelease() {
  if (!/^[a-f0-9]{64}$/i.test(lock.manifestSha256)) {
    throw new Error("play-runtime.lock.json ainda não contém o SHA-256 da primeira release Play!.");
  }
  ...
  if (manifest.version !== lock.version) {
    throw new Error("Manifesto Play! incompatível com o lock.");
  }
