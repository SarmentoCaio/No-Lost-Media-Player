...
interface RequestEnvelope extends Envelope { requestId: string; }

export type PlayCommand =
  | (RequestEnvelope & { type: "load"; source: RomSource })
  | (RequestEnvelope & { type: "pause" | "resume" | "destroy" })
  | (Envelope & { type: "set-input"; frame: InputFrame })
  | (Envelope & { type: "set-volume"; volume: number });

export type PlayEvent =
  | (Envelope & { type: "bridge-ready" })
  | (Envelope & { type: "status"; state: PlayerRuntimeState; message?: string })
  | (Envelope & { type: "keyboard-input"; code: string; pressed: boolean; repeat: boolean })
...
function isInputFrame(value: unknown): value is InputFrame {
  if (!isRecord(value) || !Number.isSafeInteger(value.sequence)
      || (value.sequence as number) < 0 || !Array.isArray(value.values)) return false;
...
export function isPlayCommand(value: unknown, sessionId: string): value is PlayCommand {
  ...
  if (value.type === "set-volume") return typeof value.volume === "number" && Number.isFinite(value.volume)
...
export function isPlayEvent(value: unknown, sessionId: string): value is PlayEvent {
  ...
  if (value.type === "keyboard-input") return typeof value.code === "string"
    && typeof value.pressed === "boolean" && typeof value.repeat === "boolean";
  if (value.type === "destroyed") return value.requestId === undefined || typeof value.requestId === "string";
  if (value.type === "status") return typeof value.state === "string"
    && PLAYER_STATES.has(value.state as PlayerRuntimeState)
    && (value.message === undefined || typeof value.message === "string");
  if (value.type === "fatal-error") return typeof value.code === "string" && typeof value.message === "string";
  if ((value.type === "command-result" || value.type === "command-error") && isRequest(value)) {
    return value.type === "command-result" || (typeof value.code === "string" && typeof value.message === "string");
  }
  return false;
}
