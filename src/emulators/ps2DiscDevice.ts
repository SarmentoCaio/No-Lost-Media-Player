export interface DiscRuntimeMemory {
  HEAPU8: Uint8Array;
}
...
export class RangeDiscDevice extends BaseDiscDevice {
  ...
  static async create(url: string) {
    ...
    if (parsed.protocol !== "https:") throw new Error("A imagem PS2 remota precisa usar HTTPS.");
    ...
    const match = contentRange?.match(/^bytes 0-0\/(\d+)$/i);
    ...
    if (!identity) throw new Error("O CDN precisa expor ETag ou Last-Modified para imagens PS2.");
  ...
  private async getBlock(index: number): Promise<Uint8Array> {
    ...
    const request = fetch(this.url, {
      ...
      if (response.status !== 206) throw new Error("O CDN deixou de responder com HTTP 206.");
      const contentRange = response.headers.get("Content-Range");
      if (contentRange !== `bytes ${start}-${end}/${this.fileSize}`) {
        throw new Error("O CDN retornou um intervalo diferente do solicitado.");
      }
      const identity = response.headers.get("ETag") ?? response.headers.get("Last-Modified") ?? "";
      if (identity !== this.identity) throw new Error("A imagem PS2 mudou durante a sessão.");
      const encoding = response.headers.get("Content-Encoding");
      if (encoding && encoding !== "identity") throw new Error("O CDN não pode comprimir respostas Range de imagens PS2.");
      const bytes = new Uint8Array(await response.arrayBuffer());
      if (bytes.byteLength !== end - start + 1) throw new Error("O CDN retornou um bloco incompleto.");
  ...
  protected async readBytes(offset: number, size: number) {
    ...
    if (last + 1 < Math.ceil(this.fileSize / RangeDiscDevice.BLOCK_SIZE)) {
      void this.getBlock(last + 1).catch(() => undefined);
    }
    return result;
  }
